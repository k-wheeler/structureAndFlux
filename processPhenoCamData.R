...
#Note: This is done outside of the download_phenocam function so that we can specify a different max gcc_90 threshold (90% change)
  ...
  if(length(files)>0){
    ...
    for(yr in desiredYears){
      ...
      risingDat <- yearDat %>% filter(direction=="rising") %>%
        summarise(n=n(),risingStart=min(transition_10),risingEnd=max(transition_90),risingSd=round(sd(as.Date(transition_50)),digits=4))
      fallingDat <- yearDat %>% filter(direction=="falling") %>%
        summarise(n=n(),fallingStart=min(transition_10),fallingEnd=max(transition_90),fallingSd=round(sd(as.Date(transition_50)),digits=4))
